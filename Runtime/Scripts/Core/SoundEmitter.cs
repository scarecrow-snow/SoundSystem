...
namespace SCLib_SoundSystem
{
    ...
    public class SoundEmitter : MonoBehaviour
    {
        ...
        /// </remarks>
        public void Initialize(SoundData data, SoundManager sountManger)
        {
            if (data == null) return;
            soundManager = sountManger;
            Data = data;
        ...
        public void Play()
        {
            ...
            // キャンセル制御用トークン作成
            WaitForSoundToEnd(cts.Token).Forget(); // 非同期監視開始
        }
        ...
        private async UniTaskVoid WaitForSoundToEnd(CancellationToken cancellationToken)
        {
            ...
            {
                Debug.LogError($"SoundEmitter: 再生中にエラーが発生しました: {ex.Message}");
                // エラー発生時もプールに返却するため、ReturnToPoolSafelyを呼び出す
            }
            finally
            {
                // 再生完了後、プールに自動返却
        ...
        {
            Debug.Assert(soundManager != null, "SoundManagerが設定されていません。SoundEmitterを使用する前に、SoundManagerを初期化してください。");
            ...
            {
                Debug.LogError($"SoundEmitter: プール返却中にエラーが発生しました: {ex.Message}");
            }
